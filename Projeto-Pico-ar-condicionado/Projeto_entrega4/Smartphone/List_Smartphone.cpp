/*
   File List_Smartphone.cpp

   Class List_Smartphone stores Nodes (class Node_PC) in a linked list.

   This file has the implementation for class List_Smartphone
...
// class List_Smartphone : public ClockCalendar_PC
class List_Smartphone : public List_base
{

public:
  Node_PC *head;     // ponteiro com tamanho de endereço de um 'Node_temp_fixa'
  List_Smartphone(); // construtor -> ao construir cria objeto 'Node_PC': head = 0;
  ~List_Smartphone();
...
List_Smartphone::List_Smartphone()
{
  // setID(newID);
...
void List_Smartphone::create_first_Node(string new_ID, string new_data, string new_hora, int new_temp_ext, int new_temp_int, bool new_automatico_ou_botao)
{
...
void List_Smartphone::insertBeforeFirst(string new_ID, string new_data, string new_hora, int new_temp_ext, int new_temp_int, bool new_automatico_ou_botao)
{
  ...
  // criando um novo Nodo com ponteiro 'next' =  'head', que foi inicializado no construtor da classe 'List_Smartphone': head = 0 || (lembrando que 'head' é um ponteiro de 'Node_PC', ou seja, seu tamanho de endereço de memória é igual ao de um 'Node_PC')
...
void List_Smartphone::insertAfterLast(string new_ID, string new_data, string new_hora, int new_temp_ext, int new_temp_int, bool new_automatico_ou_botao)
{
...
void List_Smartphone::readFirst()
{
  printNode(head);
}

void List_Smartphone::insertionSort(string new_ID, string new_data, string new_hora, int new_temp_ext, int new_temp_int, bool new_automatico_ou_botao)
{
...
void List_Smartphone::listInterval(string limit_inf, string limit_sup, string hr_init = "00:00:00", string hr_end = "23:59:59")
{
  Node_PC *aux = head;
...
string List_Smartphone::TempoLigado()
{
  Node_PC *aux = head;
  string h1, h2;
  ...
  while (aux != 0) // enquanto não chega no último nodo (que aponta para '0')
  {
    ...
    if (aux->get_temp_int() == 9999) // List_Smartphone::Ligar()
    {
      h1 = aux->hora;
    }
    else
    {
      if (aux->get_temp_int() == 1111) // List_Smartphone::Desligar()
      {
        h2 = aux->hora;
        break;
      }
  ...
  // ClockCalendar_PC::ClockInterval(h1,h2);
...
void List_Smartphone::printNode(Node_PC *aux)
{
